...
import { FaGithub, FaUsers, FaStar, FaBook, FaCalendarAlt } from "react-icons/fa";
...
export default function Main() {
  ...
  const fetchUserStats = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const contributions = await fetchFromApi("userContributions", { username, token });
  
      const totalStars = repos.reduce((sum: number, repo: { stargazers_count: number }) => sum + repo.stargazers_count, 0);
      const totalForks = repos.reduce((sum: number, repo: { forks_count: number }) => sum + repo.forks_count, 0);
  
      const languagesData = repos.reduce((acc: Record<string, number>, repo: { language: string }) => {
        if (repo.language) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-500 to-indigo-600 text-white p-8">
      ...
      <div className="max-w-lg mx-auto mb-8 bg-white rounded-lg shadow-md p-6">
        <h2 className="text-2xl font-bold text-gray-800 mb-4">Enter GitHub Details</h2>
        <p className="text-gray-600 mb-4">Provide a username and optional token to fetch stats</p>
  ...
  <div className="flex items-center gap-6">
    ...
    <div>
      ...
      <p className="flex items-center gap-2 text-gray-700">
        ...
        {new Date(userData.userInfo.created_at).toLocaleDateString()}
      </p>
    </div>
  </div>
  ...
  {/* GitHub Link */}
  <div
    onClick={() => window.open(`https://github.com/${userData.userInfo.login}`, "_blank")}
  ...
  {userData.userInfo.location && (
    <p className="flex items-center gap-2 text-gray-700">
      <span className="font-semibold">Location:</span> {userData.userInfo.location}
    </p>
  )}
  {userData.userInfo.blog && (
    <div
      onClick={() => window.open(userData.userInfo.blog, "_blank")}
      className="flex items-center gap-2 text-black hover:text-blue-700 cursor-pointer"
      title="Visit Blog"
    >
      <span className="font-semibold">Site:</span> {userData.userInfo.blog}
    </div>
  )}
  {userData.userInfo.twitter_username && (
    <div
      onClick={() =>
        window.open(`https://twitter.com/${userData.userInfo.twitter_username}`, "_blank")
      }
      className="flex items-center gap-2 text-blue-500 hover:text-blue-700 cursor-pointer"
      title="Visit Twitter"
    >
      <span className="font-semibold">Twitter (formally X):</span> @{userData.userInfo.twitter_username}
    </div>
  )}
  ...
  <div
    ...
    onClick={() => window.open(`https://github.com/${username}?tab=followers`, "_blank")}
  >
    <FaUsers className="text-4xl text-black mx-auto mb-2" />
    <p className="text-lg font-semibold text-gray-800">{userData.userInfo.followers}</p>
    <p className="text-sm text-gray-600">Followers</p>
  </div>
  <div
    className="bg-purple-100 p-4 rounded-lg text-center cursor-pointer hover:shadow-lg transition-shadow"
    onClick={() => window.open(`https://github.com/${username}?tab=following`, "_blank")}
  >
    <FaUsers className="text-4xl text-black mx-auto mb-2" />
    <p className="text-lg font-semibold text-gray-800">{userData.userInfo.following}</p>
  ...
  >
    ...
    <p className="text-lg font-semibold text-gray-800">{userData.totalRepos}</p>
    <p className="text-sm text-gray-600">Repositories</p>
  </div>
  <div className="bg-purple-100 p-4 rounded-lg text-center">
    <FaStar className="text-4xl text-black mx-auto mb-2" />
    <p className="text-lg font-semibold text-gray-800">{userData.totalStars}</p>
    <p className="text-sm text-gray-600">Stars Recieved</p>
            ...
            <div className="mt-8 space-y-6">
              <div className="bg-white p-6 rounded-lg shadow-md">
                <h3 className="text-xl font-bold text-gray-800 mb-4">Commit Activity</h3>
                <div className="h-64">
